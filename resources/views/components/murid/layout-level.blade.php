...
<div x-data="{
    ...
    init() {
        if (this.endTime !== null) {
            this.startCountdown(); // Auto start countdown if endTime exists
        } else {
    ...
    startCountdown() {
        ...
        $wire.call('startGame', this.endTime); // Call start game on Livewire
    ...
    // Fungsi untuk menghentikan countdown dan reset tampilan
    finishCountdown() {
        if (!this.isFinished) {
            this.isFinished = true;
            clearInterval(this.intervalId); // Menghentikan interval
            this.display = '00:00'; // Menampilkan 00:00 setelah selesai
            this.$wire.call('countdownFinished'); // Panggil event countdown selesai
...
@foreach ($data->where('role_name', auth()->user()->getRoleNames()->first()) as $item)
    <div x-show="currentQuestion === {{ $loop->iteration }}" class="flex flex-col gap-8 lg:gap-12 xl:gap-20 items-center w-full">
        ...
        <div class="w-full flex justify-end gap-4">
            <button type="button" x-show="currentQuestion > 1" @click="previousQuestion()"
                class="rounded-md bg-indigo-600 py-2 px-8 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-indigo-700 focus:shadow-none active:bg-indigo-700 hover:bg-indigo-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                Previous
            </button>
            <button type="button" x-show="currentQuestion < totalQuestions" @click="nextQuestion()"
                class="rounded-md bg-indigo-600 py-2 px-8 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-indigo-700 focus:shadow-none active:bg-indigo-700 hover:bg-indigo-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                Next
            </button>
            <button id="submit-button" x-show="currentQuestion === totalQuestions" type="submit"
                class="rounded-md bg-indigo-600 py-2 px-8 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-indigo-700 focus:shadow-none active:bg-indigo-700 hover:bg-indigo-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                Submit
            </button>
        </div>
...
@elseif ($infoLevel === '2' || $infoLevel === '5')
    {{-- Countdown Timer --}}
    <aside
        class="fixed top-[10px] right-[10px] z-10 max-w-max md:w-1/5 p-4 bg-gray-800 rounded-md text-white shadow-lg flex flex-col items-center">
        <h2 class="text-sm font-bold mb-2">Countdown</h2>
